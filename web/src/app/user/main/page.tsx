        ...
        <div className="auth-page">
            <section className="relative mb-8 overflow-hidden rounded-dashboard-card bg-slate-950 p-6 text-white shadow-2xl shadow-slate-300/70 md:p-9">
                ...
                <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    <div>
                        <div className="typo-caption mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-blue-100 backdrop-blur">
                            <Sparkles className="size-3.5" />
                            พื้นที่ใช้งานของคุณ
                        </div>
                        <h1 className="typo-title">สวัสดี, {session.name}</h1>
                        <p className="typo-body-sm mt-3 max-w-2xl text-slate-300">ติดตามสถานการณ์สุขภาพและสิ่งแวดล้อม พร้อมเข้าถึงชุดข้อมูลที่ได้รับอนุญาต</p>
                    </div>
                    <div className="hidden rounded-2xl border border-white/10 bg-white/10 px-5 py-4 backdrop-blur-md md:block">
                        <p className="typo-caption text-slate-300">บัญชีของคุณ</p>
                        <p className="typo-label mt-1">พร้อมใช้งาน <span className="ml-2 inline-block size-2 rounded-full bg-emerald-400" /></p>
                    </div>
                </div>
            </section>
