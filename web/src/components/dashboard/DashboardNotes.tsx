...
function PatientNotes({ hdc }: { hdc: boolean }) {
    return (
        <ol className="typo-label list-decimal space-y-2 pl-5 marker:text-neutral">
            <li><p className="typo-body">รหัส ICD-10 แยกรายโรคดังต่อไปนี้</p>
            ...
            <li><p className="typo-body">การนับจำนวนผู้ป่วย{!hdc && ': ตัดซ้ำผู้ป่วยรายคนในแต่ละสัปดาห์'}</p>
            ...
            <li><p className="typo-body">นับสัปดาห์ตามระบาดวิทยา</p><p className="typo-body">(แบบวันอาทิตย์ถึงเสาร์ โดยเริ่มนับวันที่ 1 มกราคม เป็นสัปดาห์ที่ 1 เช่น วันพุธที่ 1 มกราคม จะนับสัปดาห์ที่ 1 แค่วันที่ 1-4 มกราคม เท่านั้น)</p></li>
        </ol>
    );
}
...
function AirAdvice({ advice }: { advice: [string, string[]][] }) {
    return <div className={`grid gap-2 ${advice.length > 1 ? 'lg:grid-cols-2 lg:gap-4' : ''}`}>{advice.map(([group, items]) => <div key={group}>
        <p className="typo-body text-neutral">{group} :</p>
...
function PMNotes() {
    return (
        <div className="space-y-2">
            <p className="typo-body">1. ฝุ่นละอองขนาดไม่เกิน 2.5 ไมครอน (PM2.5) เป็นฝุ่นที่มีเส้นผ่านศูนย์กลางไม่เกิน 2.5 ไมครอน เกิดจากการเผาไหม้ทั้งจากยานพาหนะ การเผาวัสดุการเกษตร ไฟป่า และกระบวนการอุตสาหกรรม สามารถเข้าไปถึงถุงลมในปอดได้ เป็นผลทำให้เกิดโรคในระบบทางเดินหายใจ และโรคปอดต่างๆ หากได้รับในปริมาณมากหรือเป็นเวลานานจะสะสมในเนื้อเยื่อปอด ทำให้การทำงานของปอดเสื่อมประสิทธิภาพลง ทำให้หลอดลมอักเสบ มีอาการหอบหืด</p>
            <h3 className="typo-section">2. ระดับของค่าเฉลี่ย 24 ชั่วโมงของฝุ่น PM2.5</h3>
            <div className="space-y-2 md:hidden">{airLevels.map(level => <section key={level.range} className="overflow-hidden rounded-2xl border border-base-300">
                <div className="flex items-start gap-2 bg-base-200 px-3 py-2">
                    ...
                    <div><h4 className="typo-section text-neutral">{level.range} · {level.meaning}</h4><p className="typo-body-sm">สี{level.colorName}</p></div>
                </div>
                <div className="typo-body-sm px-3 py-2"><AirAdvice advice={level.advice} /></div>
            </section>)}</div>
            <div className="hidden overflow-x-auto rounded-2xl border border-base-300 focus-visible:outline-2 focus-visible:outline-neutral md:block" tabIndex={0} role="region" aria-label="ตารางระดับคุณภาพอากาศ เลื่อนแนวนอนเพื่ออ่านทุกคอลัมน์">
                <table className="typo-body-sm table w-full min-w-2xl [&_th]:px-2 [&_th]:py-1 [&_td]:px-2 [&_td]:py-1">
                    ...
                    <thead className="bg-base-200 text-base-content"><tr>
                        <th scope="col" className="typo-label w-28 whitespace-normal">ระดับของค่าเฉลี่ย 24 ชั่วโมงของฝุ่น PM2.5</th>
                        <th className="typo-label" scope="col">สีที่ใช้</th>
                        <th scope="col" className="typo-label w-28 whitespace-normal">ความหมาย (ระดับคุณภาพอากาศ)</th>
                        <th className="typo-label" scope="col">ข้อควรปฏิบัติ</th>
                    </tr></thead>
                    <tbody>{airLevels.map(level => <tr key={level.range} className="align-top even:bg-base-200/40">
                        <th scope="row" className="typo-label whitespace-nowrap">{level.range}</th>
                        <td className="typo-body"><span className={`mb-1 block h-5 w-8 rounded border border-black/15 ${level.color}`} aria-hidden="true" />{level.colorName}</td>
                        <td className="typo-body">{level.meaning}</td>
                        <td className="typo-body"><AirAdvice advice={level.advice} /></td>
                    </tr>)}</tbody>
                </table>
            </div>
        </div>
            ...
            <dialog ref={dialog} id={id} className="modal p-3" aria-labelledby={`${id}-title`} aria-describedby={`${id}-description`} onClose={() => trigger.current?.focus()}>
                <div className={`modal-box flex max-h-[calc(100dvh-1.5rem)] w-full ${name === 'PM2.5' ? 'max-w-7xl' : 'max-w-5xl'} flex-col overflow-hidden rounded-2xl border border-base-300 bg-base-100 p-0 text-left text-base-content shadow-2xl motion-reduce:transition-none`}>
                    <header className="flex shrink-0 items-center justify-between gap-3 border-b border-base-300 px-4 py-2 sm:px-5">
                        <div className="flex items-center gap-3"><h2 id={`${id}-title`} className="typo-subtitle text-neutral">หมายเหตุ</h2><span className="typo-caption badge border-black bg-black px-2 text-white">{name}</span><p id={`${id}-description`} className="typo-body sr-only">รายละเอียดประกอบการอ่านข้อมูลแดชบอร์ด {name}</p></div>
                        <button type="button" className="typo-label btn btn-circle btn-ghost min-h-11 min-w-11 cursor-pointer bg-base-200 text-neutral hover:bg-base-300" aria-label="ปิดหมายเหตุ" onClick={() => dialog.current?.close()}>✕</button>
                    </header>
                    <div className="typo-body-sm min-h-0 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5" style={{ scrollbarColor: 'var(--color-base-content) var(--color-base-100)' }} tabIndex={0}>
                        {name === 'PM2.5' ? <PMNotes /> : <PatientNotes hdc={name === 'HDC'} />}
                    </div>
                </div>
                <form method="dialog" noValidate className="modal-backdrop"><button className="typo-label cursor-pointer" tabIndex={-1} aria-label="ปิดหมายเหตุจากพื้นหลัง">ปิด</button></form>
            </dialog>
        </>
    );
}
