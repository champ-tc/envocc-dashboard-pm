...
async function main() {
    ...
    assert.equal(maps.needsTambonBoundaries({}, 1), false, 'national view stays at province level after filters clear');
    ...
    try {
        ...
        assert.equal(result.top10Exceed[0].area, 'A');
        assert.equal(result.top10Level, 'province');
        assert.equal(result.provinceTrend.A.length, 3);
        assert.ok(result.regionTrend['เขตสุขภาพที่ 1']);
        ...
        assert.equal(filtered.top10Level, 'district');
        assert.equal(filtered.mapAreas.values['["B","D2"]'].max, 10);
        const districtRanking = await actions.getDashboardData({ provinces: ['A'] });
        assert.equal(districtRanking.top10Exceed[0].area, 'D1 (A)');
        const tambons = await actions.getDashboardData({ provinces: ['A'], districts: ['D1'] });
        assert.equal(tambons.mapAreas.level, 'subdistrict');
        assert.equal(tambons.top10Level, 'subdistrict');
        assert.equal(tambons.top10Exceed[0].area, 'T1 (D1, A)');
        ...
        assert.ok(sameName.regionTrend['เขตสุขภาพที่ 6'], 'numeric health-region labels are expanded for the region chart');
